...
void MD5Update(MD5_CTX *context,unsigned char *input,unsigned int inputlen){
    ...
    index=(context->count[0]>>3)&0x3F; partlen=64-index; context->count[0]+=inputlen<<3;
    if(context->count[0]<(inputlen<<3))context->count[1]++; context->count[1]+=inputlen>>29;
...
void MD5Final(MD5_CTX *context,unsigned char digest[16]){
    unsigned int index=0,padlen=0; unsigned char bits[8];
...
int EnCodeMd5(unsigned char *encrypt,unsigned char *hash){int i=0,n=0; unsigned char decrypt[16]; MD5_CTX md5; MD5Init(&md5); MD5Update(&md5,encrypt,strlen((char *)encrypt)); MD5Final(&md5,decrypt); memcpy(hash,decrypt,16);}
/*文件校验*/
char *FileSum(char *filename){
  FILE *file; MD5_CTX context; int len,i;
  static char output[33]={""}; unsigned char buffer[1024],digest[16]; char output1[32];
  if((file=fopen(filename,"rb"))==NULL){printf ("%s can't be openedn",filename);return NULL;}
  else {
    MD5Init(&context);
    while (len=fread(buffer,1,1024,file)) MD5Update(&context,buffer,len); MD5Final(&context,digest); fclose(file);
    for(i=0;i<16;i++){sprintf(&(output1[2*i]),"%02x",(unsigned char)digest[i]);sprintf(&(output1[2*i+1]),"%02x",(unsigned char)(digest[i]<<4));}
    for(i=0;i<32;i++)output[i]=output1[i];
  }
 return output;
}
